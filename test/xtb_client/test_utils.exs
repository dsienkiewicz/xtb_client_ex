defmodule XtbClient.TestUtils do
  ...
  defmodule CertificatesHelper do
    def decode_der_certificate(path) do
      cert_file =
        Path.absname(path)
        |> File.read!()
    ...
    def decode_pem_certificate(path) do
      cert_file =
        Path.absname(path)
        |> File.read!()

      [{:Certificate, cert, _}] = :public_key.pem_decode(cert_file)
      cert
    end
